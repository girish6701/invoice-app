...
import avatar from "../assets/20.jpg";

function Header(props) {
  function changeTheme() {
    props.setDarkTheme(!props.theme);
  }

  return (
    <HeaderContainer change={props.theme}>
      <Icon>
...
const HeaderContainer = styled.div`
  background-color: ${(props) => (props.change ? "#1E2139" : "#373b53")};
  height: 70px;
  display: flex;
...
const Icon = styled.div`
  background-image: linear-gradient(#7c5dfa 55%, #9277ff 45%);
  display: flex;
...
const ThemeAvatarContainer = styled.div`
  display: flex;
  align-items: center;
`;
